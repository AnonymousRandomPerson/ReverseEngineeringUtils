...
function_location = 'overlay_29_022EC62C'
function_name = 'EntityIsValid__0234EC14'
function_header = f'u8 {function_name}(struct entity *entity)'
...
original_file_path = os.path.join(ASM_FOLDER, function_location + '.s')
...
for i, line in enumerate(original_lines):
  ...
  elif line == f'\tarm_func_end {function_name}\n':
    function_end_line = i
...
if function_start_line is None or function_end_line is None or extract_function_address is None or new_file_address is None:
...
def write_inc_file(lines: List[str], file_path: str):
  ...
  for line in lines:
    if line.startswith(ARM_FUNC_START):
      ...
      if word_index >= 0 and '0x' not in line:
...
write_inc_file(original_asm_lines, os.path.join(INCLUDE_FOLDER, f'{function_location}.inc'))
